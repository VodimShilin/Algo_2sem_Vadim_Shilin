...
long long operator%(BigInteger first, long long x) {
    return first.numbers[0] % x;
}
...
int main() {
    ...
    std::vector<std::vector<long long>> change(powers_of_two[m], std::vector<long long>(powers_of_two[m], 1));
    for (int i = 0; i < powers_of_two[m]; ++i) {
        for (int j = 0; j <= i; ++j) {
            bool flag_nice = true;
            int inc = 1;
            int first = i;
            int second = j;
            while (flag_nice && inc < m) {
                ++inc;
                if ((first & 3) == (second & 3) && (first >> 1 & 1) == (first & 1)) {
                    flag_nice = false;
                }
                first >>= 1;
                second >>= 1;
    ...
    Matrix p = change;
    Matrix ans = Matrix(powers_of_two[m]);
    //BigInteger k = n - 1;
    BigInteger k = n - 1;
    while (k > 0) {
        if (k % 2 == 1) ans *= p;
    ...
    long long to_ans = 0;
    for (int i = 0; i < powers_of_two[m]; ++i) {
        for (int j = 0; j < powers_of_two[m]; ++j) {
            to_ans += ans.matr[i][j];
            //to_ans %= mod;
        }
    }
